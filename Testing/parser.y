...
|	ConditionalOrExpression QUEST Expression COLON ConditionalExpression 										{vec = {$1,createnode("QUEST__" + *$2 , emp),$3,createnode("COLON__" + *$4 , emp),$5}; $$ = createnode("ConditionalExpression", vec);}
	;
...
int nodecnt(Node* node){
    if(cnt2.find(node)!=cnt2.end()){
        return cnt2[node];
    }
    cnt2[node]=cnt[node->val];
    cnt[node->val]++;
    return cnt2[node];
}
...
int main(){
    freopen("temp.java", "r", stdin);
    freopen("gen.dot", "w", stdout);
    yyparse();
    cout<<"strict digraph {\n";
