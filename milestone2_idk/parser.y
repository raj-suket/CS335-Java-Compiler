...
vector<pair<string, tab_item> > sym_table;
...
ClassDeclaration:
	Modifiers CLASS DIdentifier ClassExtends {incr_scope();} ClassBody 										{hide_scope(); vec = {$1,createnode("CLASS__" + *$2 , emp),$3,$4,$6}; $$ = createnode("ClassDeclaration", vec);}
|	CLASS DIdentifier ClassExtends {incr_scope();} ClassBody 										{hide_scope(); vec = {createnode("CLASS__" + *$1 , emp),$2,$3,$5}; $$ = createnode("ClassDeclaration", vec);}
|	Modifiers CLASS DIdentifier {incr_scope();} ClassBody 										{hide_scope(); vec = {$1,createnode("CLASS__" + *$2 , emp),$3,$5}; $$ = createnode("ClassDeclaration", vec);}
|	CLASS DIdentifier {incr_scope();} ClassBody 										{hide_scope(); vec = {createnode("CLASS__" + *$1 , emp),$2,$4}; $$ = createnode("ClassDeclaration", vec);}
	;
...
VariableDeclaratorId:
	DIdentifier 										{vec = {$1}; $$ = createnode("VariableDeclaratorId", vec);}
...
MethodDeclaration:
	MethodHeader {incr_scope();} MethodBody 										{hide_scope(); vec = {$1,$3}; $$ = createnode("MethodDeclaration", vec);}
	;
...
MethodDeclarator:
	DIdentifier LRB {incr_scope();} FormalParameterList {current_scope.pop_back(); counts[current_scope]--;} RRB 										{vec = {$1,createnode("LRB__" + *$2 , emp),$4,createnode("RRB__" + *$6 , emp)}; $$ = createnode("MethodDeclarator", vec);}
|	DIdentifier LRB RRB										{vec = {$1,createnode("LRB__" + *$2 , emp),createnode("RRB__" + *$3 , emp)}; $$ = createnode("MethodDeclarator", vec);}
...
ConstructorDeclaration:
	Modifiers ConstructorDeclarator {incr_scope();} Throws ConstructorBody 										{hide_scope(); vec = {$1,$2,$4,$5}; $$ = createnode("ConstructorDeclaration", vec);}
|	ConstructorDeclarator {incr_scope();} Throws ConstructorBody 										{hide_scope(); vec = {$1,$3,$4}; $$ = createnode("ConstructorDeclaration", vec);}
|	Modifiers ConstructorDeclarator {incr_scope();} ConstructorBody 										{hide_scope(); vec = {$1,$2,$4}; $$ = createnode("ConstructorDeclaration", vec);}
|	ConstructorDeclarator {incr_scope();} ConstructorBody 										{hide_scope(); vec = {$1,$3}; $$ = createnode("ConstructorDeclaration", vec);}
	;

ConstructorDeclarator:
	DIdentifier LRB {incr_scope();} FormalParameterList {current_scope.pop_back(); counts[current_scope]--;} RRB 										{vec = {$1,createnode("LRB__" + *$2 , emp),$4,createnode("RRB__" + *$6 , emp)}; $$ = createnode("ConstructorDeclarator", vec);}
|	DIdentifier LRB RRB 										{vec = {$1,createnode("LRB__" + *$2 , emp),createnode("RRB__" + *$3 , emp)}; $$ = createnode("ConstructorDeclarator", vec);}
	;
...
void typeCheckDfs(Node* root){
	if(bin_ops.find(root->val)!=to_do.end()){
		typeCheck(root);
		return;
	}
	for(auto child:root->children){
